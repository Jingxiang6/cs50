...
int main(int argc, char *argv[])
{
    ...
    *out_bf = bf;
    ...
    if (f < 1.0)
    {
        int denominator = 0;
        float numerator;
        // convert decimal f into fraction with numerator and denominator
        do
        {
            denominator ++;
            numerator = denominator * f;
        }
        while(fmod(numerator, 1.0) != 0.0);

        // iterate over infile's scanlines
        for (int i = 0, biHeight = abs(bi.biHeight); i < biHeight; i += denominator)
        {
            // change denominator lines to numerator lines
            for (int l =0; l < numerator; l ++)
            {
                // skip over the first is scanlines in infile
                fseek(inptr, 54 + (bi.biWidth * 3 + padding) * i, SEEK_SET);

                // iterate over pixels in scanline
                for (int j = 0; j < bi.biWidth; j +=denominator)
                {
                    // temporary storage
                    RGBTRIPLE triple;
                    ...
                    for (int m =0; m < numerator; m ++)
                    {
                        // write RGB triple to outfile
                        fwrite(&triple, sizeof(RGBTRIPLE), 1, outptr);
                    }
                    // skip over (denominator - 1 ) RGBTRIPLE in scanline each iteration
                    fseek(inptr, sizeof(RGBTRIPLE) * (denominator - 1), SEEK_CUR);
